...
class MolGen(nn.Module):
    ...
    def forward(self, A_fc, A_sc, age, gender):
        ...
        prop2 = F.sigmoid(self.prop_pred2(z_prop2).view(-1))

        loss_prop_rec = 10 * self.bce(age, prop1) + 1 * (gender - prop2) ** 2
    ...
    def generate(self, z_fc, z_sc, z_joint):
        # Generate with the model decoder from specified z    
        # z usually comes from sampling the prior distribution (Gaussian)    
        ...
        return A_fc_gen, A_sc_gen

    def sample(self):  
    
        # sample from Gaussian
        z_fc = torch.randn(torch.Size((self.nnodes, self.nlatent))).to(device)
        z_sc = torch.randn(torch.Size((self.nnodes, self.nlatent))).to(device)
        z_joint = torch.randn(torch.Size((self.nnodes, self.nlatent))).to(device)

        # Generate data samples
        A_fc_gen, A_sc_gen = self.generate(z_fc, z_sc, z_joint)
        ...
        return A_fc_gen, A_sc_gen, prop1, prop2
