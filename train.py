...
train_indices, val_indices = train_test_split(np.arange(len(ids)), test_size=0.2, random_state=42)
...
for epoch in range(epochs):
    ...
    for i in train_indices:
        ...
        loss = loss_graph_rec + 0 * loss_prop_rec + 0 * loss_kl+ 0 * loss_kl_disentangle + 0 * loss_norm
        loss.backward()
        optimizer.step()
    ...
    masks1.append(mask1.tolist())
    # print(mask1.shape)
    masks2.append(mask2.tolist())
    ...
    print(str(epoch)+' train_acc1:'+str(train_acc1)+' val_acc1:'+str(val_acc1)+' train_acc2:'+str(train_acc2)+' val_acc2:'+str(val_acc2)+' loss:'+str(total_loss/len(ids))+' disentangle kl:'+str(total_dis_kl/len(ids))+' kl:'+str(total_kl/len(ids))+' rec:'+str(total_rec/len(ids))+' property rec:'+str(total_prop_rec/len(ids)))
    
    torch.save({'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
                }, "model_0.pt")

with open('mask1.json','w') as f:
    json.dump(masks1,f)
    f.close()
